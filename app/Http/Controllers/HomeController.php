...
class HomeController extends Controller
{
    ...
    public function CheckLogin(Request $request)
    {
        ...
        {
            Session::flash('_msg','Incorrect Loggin');
    ...
    public function addnewDriver(Request $request)
    {
        ...
        $driver->save();
        Session::flash('_msg','Driver Added Successfully');
        Session::flash('_msgT',1);
        return back();
    }
...
//        return $request;
        ...
        {
            Session::flash('_msg','Duplicated Email!');
            Session::flash('_msgT',2);
            return back();
        }
        ...
        Session::flash('_msg','Driver Updated Successfully');
        Session::flash('_msgT',1);
    ...
    public function reportbydatesub(Request $request)
    {
        ...
        $s = Carbon::parse($request->startdate);
        $d = Carbon::parse($request->enddate);
...
//        return $request;
        ...
        $driver->delete();
        $user->delete();
        Session::flash('_msg','Driver Deleted Successfully');
        Session::flash('_msgT',1);
        return back();
    }
    ...
    public function paydriver(User $user, Drive $id)
    {
        ...
        Session::flash('_msg','Payed Successfully');
        Session::flash('_msgT',1);
...
//        return $request;
        ...
        }else
        {
            if (Auth::attempt(array('username' => $request->username, 'password' => $request->oldpassword))) {
                if ($request->confnewpassword == $request->newpassword) {
                    ...
                    Session::flash('_msg', 'New Password and Confirm Not Matched!');
                    Session::flash('_msgT', 2);

                    return back();
                }
            }else {
                Session::flash('_msg', 'Input Password Incorrect!');
                Session::flash('_msgT', 2);
        ...
        Session::flash('_msg','Update Successfully');
        Session::flash('_msgT',1);
